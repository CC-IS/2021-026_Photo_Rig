...
public:
  ...
  void run(signed int (*prof)(float)){
...
//    }
    ...
    if(millis()>oscTmr && running && useProfile){
      if(oscFunc) setSpeed(abs(oscFunc(oscillator)));
      if(oscFunc(oscillator) < 0) digitalWrite(dirPin,!dir);
      else digitalWrite(dirPin,dir);
      oscillator+=oscIncrement;
      if(oscillator>=oscMax) oscillator=0;
      oscTmr = millis() + incrementInterval;
      counts -= incrementInterval * pps/1000. * ((oscFunc(oscillator) < 0)?-1:1);
    }
  }
};
